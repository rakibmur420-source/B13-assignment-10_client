...
export default function WriterProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setStats(prev => ({ ...prev, purchased: s.data?.length || prev.purchased, bookmarks: b.data?.length || prev.bookmarks }));
      setBookmarks(b.data || []);
    }).catch(() => {});
  }, [user, loading]);
        ...
        <div className="bg-navy-light border border-gold/10 rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className="flex items-center gap-1 px-2.5 py-1 bg-gold/10 border border-gold/20 rounded-full text-xs text-gold font-medium">
                    👤 Writer
                  </span>
            ...
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => router.push("/dashboard/writer/books")}
              ...
              <button onClick={() => router.push("/dashboard/writer/bookmarks")}
                className="flex items-center gap-1 px-3 py-2 bg-navy border border-gold/20 hover:border-gold/40 text-gray-300 rounded-xl text-xs font-medium transition-all">
                ❤️ Bookmarks
              </button>
              {editMode ? (
                <button onClick={handleSave}
                  className="flex items-center gap-1 px-4 py-2 bg-gold text-navy rounded-xl text-xs font-bold transition-all">
                  ✓ Save Changes
                </button>
              ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-navy-light border border-gold/10 rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {[
                { icon: "📘", label: "Books Read", value: stats.booksRead, bg: "bg-blue-500/10", color: "text-blue-400" },
                { icon: "🟢", label: "Purchased", value: stats.purchased, bg: "bg-green-500/10", color: "text-green-400" },
                { icon: "❤️", label: "Bookmarks", value: bookmarks.length || stats.bookmarks, bg: "bg-red-500/10", color: "text-red-400" },
                { icon: "🕒", label: "Last Active", value: "2 hours ago", bg: "bg-purple-500/10", color: "text-purple-400" },
              ].map(s => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-navy-light border border-gold/10 rounded-2xl p-5">
            ...
            <div className="space-y-0">
              {[
                { label: "User ID", value: (user?._id || "6a259aa6ec90...").slice(0, 14) + "..." },
          ...
          {/* Quick Links */}
          <div className="space-y-3">
            <div onClick={() => router.push("/dashboard/writer/books")}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-1">View all your published ebooks</p>
                </div>
                <span className="text-3xl">📚</span>
              </div>
              <span className="text-xs text-gold mt-3 inline-block hover:underline">Go to Library →</span>
            </div>
            <div onClick={() => router.push("/dashboard/writer/bookmarks")}
              className="bg-purple-600 hover:bg-purple-700 rounded-2xl p-5 cursor-pointer transition-all">
              <div className="flex items-center justify-between">
                <div>
              ...
              <span className="text-xs text-purple-200 mt-3 inline-block hover:underline">View Bookmarks →</span>
            </div>
            <div onClick={() => router.push("/dashboard/writer")}
              ...
              <span className="text-xs text-green-100 mt-3 inline-block hover:underline">Go to Writer Dashboard →</span>
            </div>
          </div>
        </div>
      </main>

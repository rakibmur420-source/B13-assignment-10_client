...
export default function WriterBooksPage() {
  ...
  const fetchBooks = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/ebooks/writer/${userId}`, { headers: { Authorization: `Bearer ${token}` } });
      setBooks(res.data || []);
    ...
    <div className="flex bg-navy min-h-screen">
      ...
      <main className="flex-1 pt-20 px-4 md:px-8 pb-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => router.push("/dashboard/writer/add")} className="px-4 py-2 bg-gold hover:bg-yellow-500 text-navy font-bold rounded-xl text-sm">+ Add New Book</button>
        </div>
        {fetching ? (
          ...
          <div className="bg-navy-light border border-gold/10 rounded-2xl p-16 text-center">
            ...
            <button onClick={() => router.push("/dashboard/writer/add")} className="px-6 py-2 bg-gold text-navy font-bold rounded-xl text-sm mt-4">Add Your First Book</button>
          </div>
        ) : (
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => toggleStatus(b._id)} className={`px-3 py-1 rounded-lg text-xs font-bold ${b.status === "published" ? "bg-yellow-500/20 text-yellow-400" : "bg-green-500/20 text-green-400"}`}>{b.status === "published" ? "Unpublish" : "Publish"}</button>
                      <button onClick={() => deleteBook(b._id)} className="px-3 py-1 rounded-lg text-xs font-bold bg-red-500/20 text-red-400">Delete</button>
                    </div>
                  </div>
                </div>
              </div>

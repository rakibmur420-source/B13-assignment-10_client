        ...
        <div className="bg-navy-light border border-gold/10 rounded-2xl overflow-hidden">
          {fetching ? <div className="p-10 text-center text-gray-400">Loading...</div> : bookmarks.length === 0 ? (
            ...
            <table className="w-full">
              <thead><tr className="border-b border-gold/10">{["Title","Author","Price","Date","Action"].map(h => <th key={h} className="text-left text-gray-400 text-xs px-6 py-4 font-medium">{h}</th>)}</tr></thead>
              <tbody>
                {bookmarks.map(b => (

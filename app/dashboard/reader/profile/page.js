        ...
        <div className="bg-navy-light border border-gold/10 rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => router.push("/dashboard/reader/purchased")}
              ...
              <button onClick={() => router.push("/dashboard/reader/bookmarks")}
                className="flex items-center gap-1 px-3 py-2 bg-navy border border-gold/20 hover:border-gold/40 text-gray-300 rounded-xl text-xs font-medium transition-all">
                ❤️ Bookmarks
              </button>
              {editMode ? (
                <button onClick={handleSave}
                  className="px-4 py-2 bg-gold text-navy rounded-xl text-xs font-bold">
                  ✓ Save Changes
                </button>
              ) : (
                <button onClick={() => setEditMode(true)}
                  className="px-3 py-2 bg-navy border border-gold/20 hover:border-gold/40 text-gray-300 rounded-xl text-xs font-medium transition-all">
                  ✏️ Edit Profile
                </button>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-navy-light border border-gold/10 rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {[
                { icon: "📘", label: "Books Read", value: 24, bg: "bg-blue-500/10", color: "text-blue-400" },
                { icon: "🟢", label: "Purchased", value: purchases.length || 18, bg: "bg-green-500/10", color: "text-green-400" },
                { icon: "❤️", label: "Bookmarks", value: bookmarks.length || 7, bg: "bg-red-500/10", color: "text-red-400" },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-navy-light border border-gold/10 rounded-2xl p-5">
            ...
            <div className="space-y-0">
              {[
                { label: "User ID", value: (user?._id || "6a30f9458722...").slice(0, 14) + "..." },
          ...
          {/* Quick Links */}
          <div className="space-y-3">
            <div onClick={() => router.push("/dashboard/reader/purchased")}
            ...
            <div onClick={() => router.push("/dashboard/reader/bookmarks")}
